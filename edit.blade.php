        ...
        <div class="max-w-4xl mx-auto">
            <div class="bg-white shadow-xl rounded-lg overflow-hidden">
                <div class="border-b border-gray-200 bg-slate-800 px-8 py-6">
                    <h1 class="text-3xl font-serif text-white text-center">Edit College</h1>
                </div>

                <div class="p-8">
                    <form action="{{ route('colleges.update', ['college' => $college->id]) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        
                        <div>
                            <label for="colName" class="block text-sm font-medium text-slate-700">College Name</label>
                            <input value="{{($college->colName) }}" type="text" name="colName" id="colName"
                        ...
                        <div>
                            ...
                            <input value="{{($college->colShortName) }}" type="text" name="colShortName" id="colShortName"
                                class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-800 bg-white p-2.5"
                                required>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-800 bg-white p-2.5">{{ ($college->description), old('description'), }}</textarea>
                        </div>

                        <div>
                            <label for="colLocation" class="block text-sm font-medium text-slate-700">College Location</label>
                            <input value="{{($college->colLocation) }}" type="text" name="colLocation" id="colLocation"
                                class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-800 bg-white p-2.5">
                        </div>

                        <div>
                            <label for="telNo" class="block text-sm font-medium text-slate-700">Telephone Number</label>
                            <input value="{{($college->telNo) }}" type="text" name="telNo" id="telNo"
                                class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-800 bg-white p-2.5">
                        </div>

                        <div>
                            <label for="localNo" class="block text-sm font-medium text-slate-700">Local Number</label>
                            <input value="{{($college->localNo) }}" type="text" name="localNo" id="localNo"
                                class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-800 bg-white p-2.5">
                        </div>
